...
def pyramid_down(image, device="cpu"):
    ...
    multiband = [F.conv2d(image[:, i:i + 1, :, :], gaussian_k, padding=2, stride=2) for i in range(image.size()[1])]
...
def pyramid_up(image, device="cpu"):
    ...
    multiband = [F.conv2d(upsample[:, i:i + 1, :, :], gaussian_k, padding=2) for i in range(image.size()[1])]
...
def laplacian_pyramid(original, n_pyramids, device="cpu"):
    ...
    for i in range(len(pyramids) - 1):
        upsampl_pyramid = pyramid_up(pyramids[i + 1], device=device)
        if upsampl_pyramid.shape != pyramids[i].shape:
            upsampl_pyramid = F.interpolate(upsampl_pyramid, size=(pyramids[i].size(2), pyramids[i].size(3)),
                                            align_corners=False, mode='bilinear').float()

        diff = pyramids[i] - upsampl_pyramid
...
def extract_patches(pyramid_layer, slice_indices,
    ...
    x = (x - mean) / (std + 1e-8)
    # reshape to 2rank
    x = x.reshape(-1, x.shape[1] * slice_size * slice_size)
    return x
